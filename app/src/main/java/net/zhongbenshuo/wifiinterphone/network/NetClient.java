package net.zhongbenshuo.wifiinterphone.network;

import com.google.gson.Gson;
import com.google.gson.GsonBuilder;

import net.zhongbenshuo.wifiinterphone.BuildConfig;
import net.zhongbenshuo.wifiinterphone.WifiInterPhoneApplication;
import net.zhongbenshuo.wifiinterphone.constant.ApkInfo;
import net.zhongbenshuo.wifiinterphone.constant.NetWork;
import net.zhongbenshuo.wifiinterphone.network.retrofit.ProgressListener;
import net.zhongbenshuo.wifiinterphone.network.retrofit.ProgressResponseBody;
import net.zhongbenshuo.wifiinterphone.utils.LogUtils;
import net.zhongbenshuo.wifiinterphone.utils.NetworkUtil;
...
public class NetClient {
    ...
    private NetClient(String baseUrl) {
        ...
        //设置缓存目录
        File cacheFile = new File(WifiInterPhoneApplication.getInstance().getExternalCacheDir(), CACHE_NAME);
        //生成缓存，50M
        Cache cache = new Cache(cacheFile, 1024 * 1024 * 50);
        //缓存拦截器
        Interceptor cacheInterceptor = (chain) -> {
            Request request = chain.request();
            if (NetworkUtil.isNetworkAvailable(WifiInterPhoneApplication.getInstance())) {
                //网络可用,强制从网络获取数据
