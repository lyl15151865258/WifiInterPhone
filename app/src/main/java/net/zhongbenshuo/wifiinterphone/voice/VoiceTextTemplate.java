...
public class VoiceTextTemplate {
    ...
    private static List<String> genReadableMoney(String numString) {
        ...
        if (!TextUtils.isEmpty(numString)) {
            if (numString.contains(VoiceConstant.DOT_POINT)) {
    ...
    private static List<String> readIntPart(String integerPart) {
        ...
        for (int i = 0; i < len; i++) {
            ...
            if (current == '拾') {
                result.add(VoiceConstant.TEN);
            } else if (current == '佰') {
                result.add(VoiceConstant.HUNDRED);
            } else if (current == '仟') {
                result.add(VoiceConstant.THOUSAND);
            } else if (current == '万') {
                result.add(VoiceConstant.TEN_THOUSAND);
            } else if (current == '亿') {
                result.add(VoiceConstant.TEN_MILLION);
            } else {
